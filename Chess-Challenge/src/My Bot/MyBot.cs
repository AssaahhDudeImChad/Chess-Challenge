...
{   
    ...
    public Move Think(Board board, Timer timer){  
        ...
        Move get_best_move(bool white){
            ...
            int check_random(){
                int num = random.Next(0, legal_moves.Length);
                if(max == 0){

                    Move move = legal_moves[num];
                    int move_type = (int) move.MovePieceType;
                    if(move_type == 1){
                        Console.WriteLine("returning pawn move");
                        return num;

                    }else{
                        Console.WriteLine("Reutrning");
                        check_random();
                        return 0;
